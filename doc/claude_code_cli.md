...
- **Read**: 이미지(PNG/JPG 등)를 **시각적 콘텐츠로 봄**(아래 참고), PDF(10p 초과 시 `pages` 범위·한 번에 20p), Jupyter 셀+출력. 토큰 초과 시 `PARTIAL view`로 첫 페이지만(`offset`/`limit`으로 더). 디렉토리는 못 읽음(→ Bash `ls`)

> **"이미지를 시각적 콘텐츠로 본다"는?**
> 이미지의 이진 데이터(raw bytes)를 텍스트로 넘기는 게 아니라, **Claude(멀티모달 모델)가 그림을 실제로 눈으로 보듯 이해**하도록 비전 입력으로 변환해 전달한다는 뜻입니다.
> - 덕분에 **스크린샷 디버깅**("이 에러 화면 뭐가 문제야?"), **목업→코드 변환**("이 목업대로 CSS 만들어줘"), **다이어그램·차트 분석**이 가능
> - 단, **원본 그대로는 아님**: 모델 크기 제한에 맞춰 큰 이미지는 **자동 리사이즈·재압축**, 그 후에도 500KB 초과면 **품질 낮춘 JPEG로 재인코딩**(픽셀 치수는 유지) → **세밀한 픽셀 디테일은 놓칠 수 있음**
> - 디테일이 중요하면 Bash로 `ImageMagick` 등을 써서 **관심 영역만 먼저 잘라(crop)** 보면 선명하게 확인 가능
