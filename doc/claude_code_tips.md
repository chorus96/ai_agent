...
둘 다 "셸 명령/스크립트를 돌린다"는 점은 같지만, **누가·언제 실행하느냐**가 근본적으로 다릅니다.

> - **Hooks** = **이벤트가 발생하면 자동으로 실행**되는 셸 명령. LLM 선택과 무관하게 **항상 실행 보장**(결정론적)
> - **스크립트 실행** = **Claude가 판단하거나 사용자가 요청할 때** Bash 도구로 돌리는 명령. **실행될 수도, 안 될 수도** 있음(비결정론적)

### 상세 비교

| 항목 | **Hooks** | **스크립트 실행 (Bash 도구)** |
| --- | --- | --- |
| **트리거** | 라이프사이클 **이벤트**(PreToolUse, PostToolUse, Stop 등) | Claude의 판단 or 사용자 요청 |
| **실행 보장** | ✅ **항상**(조건 충족 시 반드시) | ❌ Claude가 "안 해도 된다"면 스킵 |
| **성격** | **결정론적**(고정 규칙) | **비결정론적**(모델 재량) |
| **설정 위치** | `settings.json`의 `hooks` 블록 | 그때그때 명령, 또는 skill/CLAUDE.md 지시 |
| **누가 실행** | **하네스(Claude Code)** 가 직접 | Claude가 **Bash 도구**로 |
| **차단 능력** | `PreToolUse`가 도구 호출을 **막을 수 있음**(exit 2) | 막지 못함(이미 실행) |
| **입력/출력** | stdin JSON, 종료 코드로 통신 | 일반 명령 출력 |
| **컨텍스트 소비** | Claude 컨텍스트 밖에서 실행 | 명령·출력이 컨텍스트에 들어옴 |

### 왜 이 차이가 중요한가

**CLAUDE.md에 "커밋 전 항상 테스트 실행"이라고 써도** → Claude가 **잊거나 건너뛸 수 있습니다**(권고일 뿐).
**같은 걸 Hook으로 만들면** → 커밋 시도(도구 호출)마다 **하네스가 강제로 실행**합니다.

> "반드시 일어나야 하는 일"은 **모델에게 부탁(스크립트 실행)하지 말고 Hook으로 강제**해야 합니다.
...
[스크립트 실행 — 재량]
사용자: "이 파일 포맷해줘"
→ Claude가 판단해서 prettier 실행 (안 할 수도 있음)

[Hook — 강제]
PostToolUse + Edit|Write matcher
→ Claude가 파일을 편집할 때마다 하네스가 자동으로 prettier 실행
  (Claude가 원하든 말든, 편집이 있으면 무조건)
```
...
| 상황 | 선택 |
| --- | --- |
| 매 편집 후 **항상** 포맷팅 | **Hook** (PostToolUse) |
| 민감 파일 편집 **항상** 차단 | **Hook** (PreToolUse, exit 2) |
| 커밋 전 **항상** 린트/테스트 | **Hook** |
| 입력 대기 시 **항상** 알림 | **Hook** (Notification) |
| 이번 한 번 빌드/테스트 실행 | **스크립트 실행** (Bash) |
| 상황 봐서 유연하게 판단 | **스크립트 실행** (또는 Skill) |

### 세 층위로 정리

- **CLAUDE.md/Skill** = "권고·절차" (모델이 따르려 하지만 강제 아님)
- **스크립트 실행(Bash)** = "그때그때 실행" (모델 재량)
- **Hooks** = "이벤트 기반 강제 실행" (하네스가 보장)

### 핵심 정리

> **Hooks = "코드로 못 박은 자동화", 스크립트 실행 = "그때그때 시키는 작업"**.
> "항상 일어나야 하는 일"이면 Hook, "이번에 한 번 하는 일"이면 스크립트 실행입니다.

> **📦 하네스(harness)란?**
> 위에서 "하네스가 강제로 실행"이라고 할 때의 **하네스 = Claude Code 자체(런타임/엔진)** 입니다. LLM(모델)을 감싸서 실제로 "일하는 에이전트"로 만들어주는 **바깥 껍데기 전체**를 말합니다.
> - **모델 = "두뇌"**(텍스트 생성만), **하네스 = "몸통·손발·신경계"**(도구 실행·권한 검사·컨텍스트 관리·Hooks 실행 등 실제 실행 인프라)
> - **Hooks는 하네스가 제공하는 여러 기능 중 하나** — 즉 "하네스 vs Hooks"는 대등한 비교가 아니라 **포함 관계**("자동차 vs 브레이크")
> - Hooks가 **결정론적으로 보장**되는 이유가 바로 이것: **모델이 아니라 하네스가 직접 실행**하기 때문
>
> ```
> 하네스 (Claude Code 런타임 = 전체)
> ├── 에이전트 루프  ├── 도구 실행(Bash/Read/Edit)  ├── 권한 시스템
> ├── 컨텍스트 관리(CLAUDE.md·메모리)  ├── Hooks 엔진 ←── 여기
> └── MCP · Skills · 서브에이전트
> ```
>
> 참고로 **모델을 잘 부리도록 이 하네스(도구·권한·컨텍스트·Hooks·서브에이전트)를 설계·튜닝하는 것**을 **"하네스 엔지니어링(Agent Harness Engineering)"** 이라 부릅니다.
...
**cron**은 유닉스/리눅스의 **반복 작업 스케줄러**이고, **cron 표현식**은 "언제 실행할지"를 **공백으로 구분한 5개 필드**로 적는 표준 문법입니다. Claude Code의 스케줄된 작업이 내부적으로 이 표현식으로 일정을 표현합니다.
...
│ │ ┌───── 일 (1-31)
│ │ │ ┌───── 월 (1-12)
│ │ │ │ ┌───── 요일 (0-6, 일요일=0 또는 7, 토=6)
│ │ │ │ │
* * * * *
```

각 필드 기호: `*`(모든 값) · 숫자(특정 값) · `*/n`(n마다) · `a-b`(범위) · `a,b,c`(목록)
...
> 읽는 법: `0 9 * * 1-5` → "0분·9시·(일 무관)·(월 무관)·월~금" = 평일 오전 9시.
> **1분 단위**가 최소 세분성(초 없음), 모든 시간은 **현지 시간대**로 해석. `L`·`W`·`MON` 같은 확장 구문은 미지원.
> `CLAUDE_CODE_DISABLE_CRON=1`로 스케줄러 전체 비활성화 가능.
